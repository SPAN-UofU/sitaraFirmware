...
const uint8_t chan_idx[NUM_CH_SUPPORTED];
 ...
 * no return value
 */
void dwt_setgpiodirection(uint32_t gpioNum, uint32 direction)
{
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
uint32_t _dwt_otpprogword32(uint32 data, uint16 address)
{
    uint8_t rd_buf[1];
    uint8_t wr_buf[4];
    uint8_t otp_done;
